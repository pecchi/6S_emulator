...
def main():
    ...
    if wavelength:
        ...
        if len(wavelength) > 2:
            print(
                'wavelength can be scalar or 2-tuple only, was given wavelength: ', wavelength)
            sys.exit(1)
        ...
        if len(wavelength) == 2:
            ...
            if spectral_filter:
                ...
                if abs(l-n) > 1e-6:
                    print('Filter must be in 2.5 nm intervals, expected {} samples, got {}'.format(
                        round(n), l))
                    sys.exit(1)
                else:
                    spectrum = Wavelength(
                        start_wavelength, end_wavelength=end_wavelength, filter=spectral_filter)
            else:
                spectrum = Wavelength(
                    start_wavelength, end_wavelength=end_wavelength)
        else:
    ...
    if os.path.isfile(config['filepath']):
        print('LUT file already exists, skipping build for: ' +
              config['filepath'])
    else:
        print('Building LUT:\n'+config['filepath'])
        build_LUT(config)
