...
class Astronomical:
    ...
    Contains the astronomical parameters required to convert at-sensor
    radiance to top-of-atmosphere (TOA) reflectance

    - ESUN = Exoatmospheric spectral irradiance

    - Earth_Sun_d = Distance between the Earth and the Sun for a given
      day of year (doy)
    ...
    def ESUNs():
        ...
        TODO include ESUN for ASTER and Sentinel 2 !!
        ...
        # adds descriptive key names
        def addDescriptiveKeyNames(keyPairs, ESUN_dict):
            for keyPair in keyPairs:
                value = ESUN_dict[keyPair[0]]
                ESUN_dict[keyPair[1]] = value

            return ESUN_dict

        # ESUN for Landsats 4,5,7 (Chader et al 2009)
        LANDSAT_4 = {'B1': 1983,
                     'B2': 1795,
                     'B3': 1539,
                     'B4': 1028,
        ...
        # ESUN for Landsat 8 (Benjamin Leutner)
        LANDSAT_8 = {'B1': 1895.33,
                     'B2': 2004.57,
                     'B3': 1820.75,
                     'B4': 1549.49,
                     'B5': 951.76,
        ...
        # define additional descriptive key names
        keyPairs_Landsats457 = [('B1', 'blue'),
                                ('B2', 'green'),
                                ('B3', 'red'),
                                ('B4', 'nir'),
                                ('B5', 'swir1'),
                                ('B6', 'tir'),
                                ('B7', 'swir2')]
        keyPairs_Landsat8 = [('B1', 'coastalaerosol'),
                             ('B2', 'blue'),
                             ('B3', 'green'),
                             ('B4', 'red'),
                             ('B5', 'nir'),
        ...
        # add descriptive key names
        LANDSAT_4 = addDescriptiveKeyNames(keyPairs_Landsats457, LANDSAT_4)
        LANDSAT_5 = addDescriptiveKeyNames(keyPairs_Landsats457, LANDSAT_5)
        LANDSAT_7 = addDescriptiveKeyNames(keyPairs_Landsats457, LANDSAT_7)
        LANDSAT_8 = addDescriptiveKeyNames(keyPairs_Landsat8, LANDSAT_8)

        # ESUN dictionary
        ESUNs = {'LANDSAT_4': LANDSAT_4,
                 'LANDSAT_5': LANDSAT_5,
                 'LANDSAT_7': LANDSAT_7,
                 'LANDSAT_8': LANDSAT_8}
    ...
    def solar_irradiance_spectrum():
        """
        FROM:
        6S FORTRAN subroutine: SOLIRR 

        Solar Irradiance (in W/m2/micron) contains the values of the solar
        irradiance between 0.25 and 4.0 microns, by step of 0.0025 microns
        total irradiance between 0.25 and 4.0 microns is 1358 W/m2
        ...
        p179 6S_Manual (part 3) states that the solar irradiance spectrum is from:
        ...
        wavelengths = {'array': np.linspace(0.25, 4.0, 1501), 'min': 0.25,
                       'max': 4.0, 'increments': 0.0025, 'num_values': 1501, 'unit': 'microns'}
        ...
        return {'solar_irradiance_per_micron': solar_irradiance, 'wavelengths': wavelengths}

    def Earth_Sun_d():
        """

        Earth-Sun distance (d) in astronomical units as a function of day-of-year (doy)

        returns dictionary {doy: d}
